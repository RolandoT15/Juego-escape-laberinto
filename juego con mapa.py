...
while running:
    ...
    # ----- MOVIMIENTO ENEMIGOS (huyen del jugador, evitando paredes) -----
    for e in enemies:
        # gestionar respawn
        if not e['visible']:
            if now >= e['respawn_time']:
                # reaparecer en su spawn
        ...
        if dist == 0:
            # empuja a enemigo aleatoriamente un poco
            ...
            # probar solo Y
                ...
                for ax, ay in attempts:
                    if can_move_rect(enemy_rect, ax, ay, for_enemy=True):
                        ...
                        moved = True
                        break
            # si no pudo moverse, queda quieto
    ...
    # Empuje jugador-enemigos y detectar toque
    for idx, er in enumerate(enemy_rects):
        push_back_entities(player_rect, er)

        # chequear toque: si jugador colisiona y enemigo visible -> desaparecer y programar respawn
        if player_rect.colliderect(er) and enemies[idx]['visible']:
            enemies[idx]['visible'] = False
            enemies[idx]['respawn_time'] = now + 3.0
            # opcional: mover fuera de pantalla hasta que reaparezca
            enemies[idx]['x'] = -1000.0
            enemies[idx]['y'] = -1000.0

    # aplicar posiciones corregidas después del empuje
    player_x, player_y = float(player_rect.x), float(player_rect.y)
